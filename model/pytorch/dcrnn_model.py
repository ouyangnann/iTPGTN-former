...
class DCRNNModel(nn.Module, Seq2SeqAttrs):
    ...
    def decoder(self, encoder_hidden_state, adj_mx, batches_seen=None, tgt=None):
        """
        Decoder forward pass
    ...
    def forward(self, inputs, adj_mx, batches_seen=None, tgt = None):
        """
        ...
        :return: output: (self.horizon, batch_size, self.num_nodes * self.output_dim)
        """
        inputs=inputs.reshape(12,64,207)
        
        encoder_hidden_state = self.encoder(inputs, adj_mx)
        self._logger.debug("Encoder complete, starting decoder")
        outputs = self.decoder(encoder_hidden_state, adj_mx, batches_seen, tgt)
        self._logger.debug("Decoder complete")
        if batches_seen == 0:
            self._logger.info(
                "Total trainable parameters {}".format(count_parameters(self))
            )
        outputs = outputs.reshape(64,12,207)
        return outputs