...
class DCGRUCell(torch.nn.Module):
    ...
    def _calculate_supports(self, adj_mx):
    ...
    def forward(self, inputs, adj_mx, hx):
        ...
        """
        if self._supports == []:
            self._calculate_supports(adj_mx.astype(np.float32))  # 在第一次调用 forward 时计算支持矩阵
            
        output_size = 2 * self._num_units
        if self._use_gc_for_ru:
            # 使用图卷积
            fn = lambda x, y: self._gconv(x, y, output_size, bias_start=1.0)
        else:
            # 使用全连接层
            fn = lambda x, y: self._fc(x, y, output_size, bias_start=1.0)

        value = torch.sigmoid(fn(inputs, hx))
    ...
    def _gconv(self, inputs, state, output_size, bias_start=0.0):
        # supports = self._calculate_supports(adj_mx)
